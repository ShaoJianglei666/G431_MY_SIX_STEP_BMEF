...
#define MIN_DEMAG_TIME                     ((uint16_t)2) /*!< Demagnetization delay in number of HF timer periods elapsed before a first
                                                           BEMF ADC measurement is processed in an attempt to detect the BEMF zero crossing */
#define SPEED_THRESHOLD_DEMAG              ((uint32_t)4000) /*!< Speed threshold above which the  RUN_DEMAGN_DELAY_MIN is applied */
...
#define SW_DEADTIME_NS                     2000 /*!< Dead-time to be inserted by FW, only if low side signals are enabled */
...
#define PID_SPEED_KP_DEFAULT          2340/(SPEED_UNIT/10) /* Workbench compute the gain for 01Hz unit*/
#define PID_SPEED_KI_DEFAULT          57/(SPEED_UNIT/10) /* Workbench compute the gain for 01Hz unit*/
...
#define DEFAULT_TARGET_SPEED_RPM           1200
